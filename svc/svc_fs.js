...
/* ах, чудна българска земьо, полюшвай цъфтящи жита! */
  ...
  function fs_init(){
    ...
    //if(user=='elefter') zint=zint_paths[0]
      fn_log  =log//'svc.'+user+'.log' // par[p].order+
    //fsh.Run( "command.com /K ...")
    } else msg('<e>ActiveX undefined outside IE/mshta!</e> cannot make file i/o.') // ff,node? -- _dui.msg()
  }

  function set_paths(){ // #pers-init.bat > user, desk, pers
     rootpath=root // --- old: 'C:\\Users\\'+user+'\\Desktop\\'
  // userpath=user // --- old: 'C:\\Users\\'+user+'\\'
     workpath=jobs // --- old: pers='C:\\Users\\'+user+'\\Desktop\\#pers\\'
  // eps, wrk	
  }
...
// ---
  ...
  // %zint% --batch --mirror --filetype=eps --cmyk -b %bt% -i %file%
     ...
     var mes="... 'svc-eps-make.cmd' start:"+start+' '+run; msg(mes); // log_apnd(mes); 
...
// open Error: File not found
  var br='<br/>'+crlf,mes
    ...
    // --- skip on empty par
	   if(undef(ActiveX) || fname== null || fname=='') {total_err++; return 0}
	   var f_=fnd_.length,r_=rep_.length, i,j,k
	   if(f_<1 || r_<1 || f_!=r_)  {total_err++; return 0}
	...
	// -- open file in trgpath
       ...
       }catch(err){ mes='... eps repl: open out:'+trgpath+fname+t_ext+' event:"'+errr+'"';
	     if(err=='Error: Permission denied') msg+='   may be this file is opened from another apl.'
	     log_apnd(mes); msg(mes)
	     total_err++; return 0
	   }
