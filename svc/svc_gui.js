...
//  msg(ht_out(buf))
    msg('saved serias '+i)//+' values:'+si_total)
	return  buf
  }
  ...
  function ser_tbl(){   // параметри за запис на серия
    ...
    {   i=1; rbuf=''// --- par	
        rbuf+=col("<input type='text'   ID='SET_order' class='e' value='?' style='width:18ch;' maxlength=16"
			+" onchange='par[p].order=this.value; new_par(); set_paths(); xpth_.innerHTML=workpath;'>")
        rbuf+=col("<input type='text'   ID='SET_id'    class='e' value='-' style='width:36ch;' maxlength=32"
			+" onchange='par[p].id=this.value;    new_par(); set_paths(); xpth_.innerHTML=workpath;'>")
        rbuf+=col(bcn_sel(),'')
